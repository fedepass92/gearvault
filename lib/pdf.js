...
async function buildQuoteDoc(quote, items, settings = {}) {
  ...
  if (logoDataUrl) {
    try {
      ...
      const logoH = 10  // ~30pt height
      const logoW = Math.min(logoH * imgProps.width / imgProps.height, 65)
      doc.addImage(logoDataUrl, imgProps.fileType.toUpperCase(), 14, 6, logoW, logoH)
      logoPlaced = true
  ...
  // GearVault tag + optional P.IVA right
  doc.setFontSize(10)
  doc.setFont('helvetica', 'normal')
  doc.setTextColor(255, 255, 255)
  ...
  if (quote.client_email) {
    ...
    doc.setFont('helvetica', 'normal')
    doc.setTextColor(71, 85, 105)
    doc.text(quote.client_email, 19, y + 22)
  }
