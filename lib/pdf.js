...
/** Quote / Preventivo PDF — portrait A4 */
export async function exportQuotePDF(quote, items) {
  ...
  doc.setTextColor(148, 163, 184)
  doc.text('GearVault — Sistema di Gestione Attrezzatura', 14, 18)
  doc.text(`Preventivo  ·  ${today}`, 14, 24)
  doc.setTextColor(255, 255, 255)
  doc.setFontSize(9)
  doc.setFont('helvetica', 'bold')
  doc.text('PREVENTIVO', W - 14, 13, { align: 'right' })
  doc.setFontSize(7.5)
  doc.setFont('helvetica', 'normal')
  doc.setTextColor(148, 163, 184)
  ...
  doc.roundedRect(W - 14 - pillW, y - 6, pillW, 7, 1.5, 1.5, 'F')
  doc.setTextColor(255, 255, 255)
  doc.setFontSize(7.5)
  doc.setFont('helvetica', 'bold')
  doc.text(statusLabel, W - 14 - pillW / 2, y - 1.5, { align: 'center' })

  y += 10

  // ── Info row (client + event date) ────────────────────────────────────────────
  const infoBoxH = 22
  const halfW = (W - 32) / 2
  ...
  doc.setFont('helvetica', 'bold')
  doc.setTextColor(100, 116, 139)
  doc.text('CLIENTE', 19, y + 5.5)
  doc.setFontSize(9)
  doc.setFont('helvetica', 'bold')
  doc.setTextColor(15, 23, 42)
  doc.text(quote.client_name || '—', 19, y + 12)
  if (quote.client_email) {
    doc.setFontSize(7.5)
    doc.setFont('helvetica', 'normal')
    doc.setTextColor(71, 85, 105)
    doc.text(quote.client_email, 19, y + 18)
  }
  ...
  doc.setFont('helvetica', 'bold')
  doc.setTextColor(100, 116, 139)
  doc.text('DATA EVENTO', rx + 5, y + 5.5)
  doc.setFontSize(9)
  doc.setFont('helvetica', 'bold')
  doc.setTextColor(15, 23, 42)
  const eventDateStr = quote.event_date
    ...
    : '—'
  doc.text(eventDateStr, rx + 5, y + 12)
  doc.setFontSize(7.5)
  doc.setFont('helvetica', 'normal')
  doc.setTextColor(100, 116, 139)
  ...
  const tableBody = items.map((item) => {
    ...
    const name = [eq.name, eq.brand && eq.model ? `${eq.brand} ${eq.model}` : (eq.brand || '')].filter(Boolean).join('\n')
  ...
  // Total row
  tableBody.push([
    { content: `TOTALE (${items.length} item)`, colSpan: 5, styles: { fontStyle: 'bold', fillColor: [15, 23, 42], textColor: 255, halign: 'right' } },
    { content: fmt(grandTotal), styles: { fontStyle: 'bold', fillColor: [37, 99, 235], textColor: 255, halign: 'right' } },
  ])
  ...
  // ── Grand total callout ───────────────────────────────────────────────────────
  const calloutW = 80
  const calloutX = W - 14 - calloutW
  doc.setFillColor(15, 23, 42)
  doc.roundedRect(calloutX, y, calloutW, 14, 2, 2, 'F')
  doc.setFillColor(37, 99, 235)
  doc.roundedRect(calloutX, y, calloutW / 2, 14, 2, 2, 'F')
  doc.rect(calloutX + calloutW / 2 - 2, y, 4, 14, 'F') // join seam
  doc.setTextColor(255, 255, 255)
  doc.setFontSize(8)
  doc.setFont('helvetica', 'bold')
  doc.text('TOTALE PREVENTIVO', calloutX + 4, y + 9)
  doc.setFontSize(10)
  doc.text(fmt(grandTotal), W - 18, y + 9, { align: 'right' })
  ...
  if (quote.notes && quote.notes.trim()) {
    ...
    doc.setFillColor(254, 243, 199)
    doc.setDrawColor(252, 211, 77)
    const noteLines = doc.splitTextToSize(quote.notes.trim(), W - 44)
    const noteBoxH = 12 + noteLines.length * 4.5
    doc.roundedRect(14, y, W - 28, noteBoxH, 2, 2, 'FD')
    doc.setTextColor(146, 64, 14)
    doc.setFontSize(7)
    doc.setFont('helvetica', 'bold')
    doc.text('NOTE', 20, y + 6)
    doc.setFont('helvetica', 'normal')
    noteLines.forEach((line, i) => doc.text(line, 20, y + 11 + i * 4.5))
    y += noteBoxH + 8
  }
