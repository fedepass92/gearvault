...
export const FROM_EMAIL = 'noreply@gear.braindigital.it'
...
// ── Base layout wrapper ────────────────────────────────────────────────────────
function layout(content, logoUrl = '') {
  const headerLeft = logoUrl
    ? `<img src="${logoUrl}" alt="" height="36" style="height:36px;display:block;max-width:180px;object-fit:contain;">`
    : `<span style="font-size:20px;font-weight:900;color:#ffffff;letter-spacing:-0.5px;">BRAIN</span>
       <span style="font-size:10px;font-weight:700;color:#ffffff;letter-spacing:4px;margin-left:6px;vertical-align:middle;opacity:0.7;">DIGITAL</span>`
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:${BODY_BG};padding:32px 16px;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        ...
        <!-- Header -->
        <tr>
          <td style="background:${BRAND_BG};border-radius:12px 12px 0 0;padding:20px 32px;">
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                ...
                <td align="right" style="vertical-align:middle;">
                  <span style="font-size:12px;color:#ffffff;font-weight:600;opacity:0.85;">GearVault</span>
                </td>
              </tr>
            </table>
          </td>
        ...
        <!-- Body card -->
        <tr>
          <td style="background:${CARD_BG};padding:36px 32px;border-left:1px solid #e2e8f0;border-right:1px solid #e2e8f0;">
            ${content}
          </td>
        </tr>
        ...
        <!-- Footer -->
        <tr>
          <td style="background:${BRAND_BG};border-radius:0 0 12px 12px;padding:20px 32px;text-align:center;">
            <p style="margin:0;font-size:11px;color:#94a3b8;">Brain Digital · GearVault — Sistema di Gestione Attrezzatura</p>
            <p style="margin:6px 0 0;font-size:10px;color:#64748b;">Questa email è stata generata automaticamente, non rispondere a questo messaggio.</p>
          </td>
        </tr>
...
export function inviteTemplate({ inviteeEmail, inviterName, loginUrl, logoUrl = '' }) {
  const content = `
    ...
    <div style="background:#f1f5f9;border:1px solid #e2e8f0;border-radius:12px;padding:20px 24px;margin:0 0 28px;">
      ...
      <p style="margin:0;font-size:15px;font-weight:700;color:${ACCENT};">${inviteeEmail}</p>
    </div>
...
export function resetPasswordTemplate({ resetUrl }) {
  const content = `
    <h1 style="margin:0 0 8px;font-size:22px;font-weight:800;color:${TEXT_MAIN};">Reimposta la tua password</h1>
    <p style="margin:0 0 24px;font-size:14px;color:${TEXT_MUTED};">
      Hai richiesto di reimpostare la password del tuo account GearVault.<br/>
    ...
    <p style="margin:28px 0 0;font-size:12px;color:#94a3b8;">
      Il link è valido per 1 ora. Se non hai richiesto il reset, ignora questa email — il tuo account è al sicuro.
    </p>
  `
  return {
    subject: 'Reimposta la tua password — GearVault',
    html: layout(content),
  }
}
