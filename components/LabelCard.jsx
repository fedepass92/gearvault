...
export const LABEL_FORMATS = [
  { id: 'keytag',  label: 'Key Tag',    size: '50×80mm'  },
  { id: 'dot',     label: 'Dot',        size: '25×25mm'  },
  { id: 'baby',    label: 'Baby Label', size: '38×25mm'  },
  { id: 'barcode', label: 'Barcode',    size: '60×25mm'  },
  { id: 'cable',   label: 'Cable Wrap', size: '80×15mm'  },
]

// ── Brain Digital logo SVG ───────────────────────────────────────────────────
function BrainLogo({ width = 60, color = '#ffffff' }) {
  return (
...
// ── 1. Key Tag  50×80mm vertical ─────────────────────────────────────────────
function LabelKeyTag({ item, isCase, isKit, qrValue }) {
  const serial = (!isCase && !isKit && item.serial_number) ? item.serial_number : item.id?.slice(0, 8).toUpperCase()
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <BrainLogo width={36} color="#ffffff" />
        {(isCase || isKit) && (
          <span style={{
            fontSize: '5px', letterSpacing: '0.18em', fontWeight: '700',
            textTransform: 'uppercase', color: isKit ? '#a78bfa' : '#93c5fd',
          }}>
            {isKit ? 'Kit' : 'Case'}
          </span>
        )}
      ...
      <div style={{
        ...
        <div style={{
          fontSize: '7.5px', fontWeight: '700', color: '#0f172a',
        ...
        <div style={{
          ...
          <QRCodeSVG value={qrValue} size={88} level="M" style={{ display: 'block' }} />
        </div>
        ...
        <div style={{
          ...
          padding: '1.5mm 3mm', textAlign: 'center',
        }}>
          {serial}
        </div>
...
// ── 2. Dot  25×25mm circular ──────────────────────────────────────────────────
function LabelDot({ item, isCase, isKit, qrValue }) {
  const serial = (!isCase && !isKit && item.serial_number)
    ? item.serial_number.slice(0, 10)
  ...
  return (
    ...
    >
      {/* QR fills the circle */}
      <QRCodeSVG value={qrValue} size={64} level="L" style={{ display: 'block' }} />
...
// ── 3. Baby Label  38×25mm ────────────────────────────────────────────────────
function LabelBaby({ item, isCase, isKit, qrValue }) {
  const serial = (!isCase && !isKit && item.serial_number) ? item.serial_number : null
    ...
    >
      ...
      <div style={{
        ...
        <div>
          {(isCase || isKit) && (
            <div style={{
              fontSize: '4.5px', fontWeight: '800', letterSpacing: '0.12em',
          ...
          <div style={{
            fontSize: '7px', fontWeight: '800', color: '#0f172a',
          ...
          {serial && (
            <div style={{
              fontSize: '5px', fontFamily: 'monospace', color: '#64748b',
        ...
        <BrainLogo width={24} color="#94a3b8" />
      </div>
    </div>
  )
}

// ── 4. Barcode Label  60×25mm ─────────────────────────────────────────────────
function LabelBarcode({ item, isCase, isKit, qrValue }) {
  const barcodeRef = useRef()
  const barcodeValue = (!isCase && !isKit && item.serial_number) ? item.serial_number : item.name
  useBarcode(barcodeRef, barcodeValue, { width: 1.4, height: 32 })

  const displayId = (!isCase && !isKit && item.serial_number)
    ? item.serial_number
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <BrainLogo width={32} color="#0f172a" />
        <div style={{
          fontSize: '6px', fontWeight: '700', color: '#0f172a',
          maxWidth: '28mm', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
          textAlign: 'right',
        }}>
          {item.name}
        </div>
      ...
      <div style={{
        ...
        <svg ref={barcodeRef} style={{ maxWidth: '100%', height: '32px' }} />
        <div style={{
          fontSize: '5px', fontFamily: 'monospace', color: '#475569',
...
// ── 5. Cable Wrap  80×15mm horizontal ─────────────────────────────────────────
function LabelCableWrap({ item, isCase, isKit, qrValue }) {
  return (
    <div
      className="label-card"
      style={{
    ...
    >
      ...
      <div style={{
        ...
        <BrainLogo width={30} color="#0f172a" />
        <div style={{
      ...
      {/* Divider */}
      <div style={{ width: '1px', background: '#e2e8f0', alignSelf: 'stretch', margin: '3mm 0' }} />
      ...
      <div style={{
        ...
        <QRCodeSVG value={qrValue} size={28} level="L" style={{ display: 'block' }} />
      </div>
    </div>
  )
}
...
export default function LabelCard({ item, format = 'keytag', isCase = false, isKit = false }) {
  ...
  if (format === 'dot')     return <LabelDot     {...props} />
  if (format === 'baby')    return <LabelBaby     {...props} />
  if (format === 'barcode') return <LabelBarcode  {...props} />
  if (format === 'cable')   return <LabelCableWrap {...props} />
  return <LabelKeyTag {...props} />
}
