...
// Inline SVG fallback — always works without network/auth
function BrainLogoSVG({ width, className }) {
  return (
    <svg width={width} viewBox="0 0 723.29 271.79" xmlns="http://www.w3.org/2000/svg" className={`block ${className || ''}`}>
      <g fill="currentColor">
...
/**
 * Company logo component — shows uploaded logo or falls back to the inline SVG.
 *
 * @param {'light'|'dark'|'svg'} variant  - 'light' for dark backgrounds, 'dark' for light backgrounds, 'svg' for app use
 * @param {number}               width    - target width in px
 * @param {string}               className
 */
export default function CompanyLogo({ variant = 'light', width = 120, className = '' }) {
  // undefined = loading, null = no logo (show SVG), string = url to show
  const [logoUrl, setLogoUrl] = useState(undefined)

  useEffect(() => {
    const KEY_MAP = { svg: 'company_logo_svg', light: 'company_logo_light', dark: 'company_logo_dark' }
    const key = KEY_MAP[variant] || 'company_logo_light'
  ...
  // While fetching: invisible placeholder to avoid layout shift
  if (logoUrl === undefined) {
    return <div style={{ width, height: Math.round(width * 0.376) }} className={className} />
  }

  if (logoUrl) {
    // eslint-disable-next-line @next/next/no-img-element
    return <img src={logoUrl} alt="Logo" width={width} style={{ width, height: 'auto' }} className={`block ${className}`} />
  }

  return <BrainLogoSVG width={width} className={className} />
}
